...
# string = "avlisj ehflasnmx"
def ceaserCipher(secretmessage):
    secretmessage = secretmessage.lower()
    secretmessage = secretmessage.capitalize()
    decode = []
    encode = ""
    for i in secretmessage:
        if i in key:
            ...
            i = i
    print(encode.join(decode))

ceaserCipher("Pnrfne pvcure? V zhpu cersre Pnrfne fnynq!")
# ceaserCipher("avlisj??? ehflasnmx")
ceaserCipher("caesar cipher? i much prefer caesar salad!")
